...
```
当函数返回或抛出错误，而离开当前函数作用域时或因为异常退出try，
...
| `a>=b`     | 大于等于比较，仅支持数值类型，调用`__gt__` `__eq__`魔术方法                                                                         |
| `a<=b`     | 小于等于比较，仅支持数值类型，调用`__lt__` `__eq__`魔术方法                                                                         |
...
| `__StopIterSignal__`                                | 特殊对象 | 作为迭代器终止的返回对象                                                                                                                    | 无                                                                           |
